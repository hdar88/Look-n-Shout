webgazer
  .setGazeListener(function (data, elapsedTime) {
    ...
    if (
      ...
      yPrediction >= topLeftRect.top &&
      yPrediction <= topLeftRect.bottom &&
      xPrediction >= topLeftRect.left &&
      xPrediction <= topLeftRect.right
    ) {
      console.log("Du schaust nach oben links");
      //TODO simulateKeyCombiPress(keys)
    } else if (
      yPrediction >= topRightRect.top &&
      yPrediction <= topRightRect.bottom &&
      xPrediction >= topRightRect.left &&
      xPrediction <= topRightRect.right
    ) {
      console.log("Du schaust nach oben rechts");
      //TODO simulateKeyCombiPress(keys)
    } else if (
      yPrediction >= bottomLeftRect.top &&
      yPrediction <= bottomLeftRect.bottom &&
      xPrediction >= bottomLeftRect.left &&
      xPrediction <= bottomLeftRect.right
    ) {
      console.log("Du schaust nach unten links");
      //TODO simulateKeyCombiPress(keys)
    } else if (
      yPrediction >= bottomRightRect.top &&
      yPrediction <= bottomRightRect.bottom &&
      xPrediction >= bottomRightRect.left &&
      xPrediction <= bottomRightRect.right
    ) {
